...
class FSkookumScriptGenerator : public ISkookumScriptGenerator
  ...
  TSet<UStruct *>       m_exported_classes; // Whenever a class gets exported, it gets added to this list
  ...
  TArray<StructRecord>  m_used_structs; // All structs used as types (by parameters, properties etc.)
...
void FSkookumScriptGenerator::FinishExport()
  ...
  // Generate any structs used but may not exported yet
  for (auto & used_struct : m_used_structs)
    {
    generate_struct(used_struct.m_struct_p, used_struct.m_source_header_file_name);
    }
  ...
  if (value_property)
    ...
    switch (type_id)
      ...
      case SkTypeID_List:
        ...
        fmt +=                 TEXT("        {\r\n");
        fmt += generate_return_or_param_value_passing(class_or_struct_p, function_p, inner_property, /*value_name=*/ FString::Printf(TEXT("t_arr_%d[i]"), ParamIndex + 1), 
...
bool FSkookumScriptGenerator::is_struct_type_supported(UStruct * struct_p) {
	if (UScriptStruct * script_struct = Cast<UScriptStruct>(struct_p)) {
		if (script_struct->StructFlags & STRUCT_RequiredAPI)
			return true;
	}
...
FString FSkookumScriptGenerator::get_skookum_class_path(UStruct * class_or_struct_p)
  ...
  while ((super_p = is_class ? Cast<UClass>(super_p)->GetSuperClass() : super_p->GetSuperStruct()) != nullptr)
    ...
    /*else
        m_used_structs.AddUnique(StructRecord(super_p, m_source_header_file_name));*/
    }

  // Build path
  int32 max_super_class_nesting = FMath::Max(m_scripts_path_depth - 1, 0);
  FString class_path = is_class ? m_scripts_path / TEXT("Object") : m_scripts_path / TEXT("Object");
...
FString FSkookumScriptGenerator::get_skookum_property_type_name(UProperty * property_p)
  ...
  if (type_id == SkTypeID_UStruct)
    ...
    m_used_structs.AddUnique(StructRecord(struct_p, m_source_header_file_name));
